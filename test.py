...
class TestKVS(unittest.TestCase):
    ...
    #     # get the address of ping leader and ping watcher from node1
    #     response = requests.get( 'http://localhost:8082/ping-check')
    #     responseInJson = response.json()
    #     leaderAddress = responseInJson["ping-leader"]
    #     watcherAddress = responseInJson["ping-watcher"]

    #     # get the instance name of ping leader and ping watcher
    #     pingLeader = int(leaderAddress.split(':')[0][-1])
    #     pingLeader -= 1
    #     pingWatcher = int(watcherAddress.split(':')[0][-1])
    #     pingWatcher -= 1

    #     print("\n###################### Disconnecting the ping watcher for 1 second ######################\n")
    #     disconnectFromNetwork(subnetName, 'node' + str(pingWatcher))
    #     time.sleep(1)
    #     connectToNetwork(subnetName, 'node' + str(pingWatcher))
    #     # get the address of ping watcher from node 2
    #     response = requests.get( 'http://localhost:8083/ping-check')
    #     responseInJson = response.json()
    #     self.assertEqual(response.status_code, 200)
    #     self.assertEqual(responseInJson['ping-watcher'], watcherAddress)

    #     print("\n###################### Disconnecting the ping leader for 1 second ######################\n")
    #     disconnectFromNetwork(subnetName, 'node' + str(pingLeader))
    #     time.sleep(1)
    #     connectToNetwork(subnetName, 'node' + str(pingLeader))
    #     # get the address of ping leader from node 3
    #     response = requests.get( 'http://localhost:8084/ping-check')
    #     responseInJson = response.json()
    #     self.assertEqual(response.status_code, 200)
    #     self.assertEqual(responseInJson['ping-leader'], leaderAddress)

        # print("\n###################### Disconnecting the ping watcher for 3 seconds ######################\n")
        # disconnectFromNetwork(subnetName, 'node' + str(pingWatcher))
        # time.sleep(20)
        # connectToNetwork(subnetName, 'node' + str(pingWatcher))
        # # get the address of ping watcher from node 4
        # response = requests.get( 'http://localhost:8085/ping-check')
        # responseInJson = response.json()
        # print(responseInJson['ping-leader'])
        # print(responseInJson['ping-watcher'])

        # print("\n###################### Disconnecting the ping leader for 3 seconds ######################\n")
        # disconnectFromNetwork(subnetName, 'node' + str(pingLeader))
        # time.sleep(3)
        # connectToNetwork(subnetName, 'node' + str(pingLeader))
        # # get the address of ping leader from node5
        # response = requests.get( 'http://localhost:8086/ping-check')
        # responseInJson = response.json()
        # print(responseInJson['ping-leader'])
        # print(responseInJson['ping-watcher'])
    ...
    ######################### KVS tests #######################################################
    def test_f_put_key_value_operation(self):
        # put a new keys in the store
        ...
        # update the values of the existing keys
        print("\n###################### Updating keys/values in the store ######################\n")
        ...
        for counter in range(self.keyCount):
            ...
            response = requests.put('http://localhost:' + nodeHostPortList[nodeIndex] + '/key-value-store/key' + str(counter), json={'value': "newValue" + str(counter), "causal-metadata": nextCausalMetadata})
            responseInJson = response.json()
            self.assertEqual(response.status_code, 200)
            nextCausalMetadata = responseInJson["causal-metadata"]
        ...
        # get the updated values of the keys
        print("\n###################### Getting updated keys/values from the store ######################\n")
        ...
        for counter in range(self.keyCount):
            ...
            self.assertEqual(response.status_code, 200)
            value = responseInJson["value"]
            self.assertEqual(value, "newValue" + str(counter))
    

    ######################### Shard tests #######################################################
    def test_g_shard_key_count(self):
    ...
    def test_i_impossible_reshard(self):
    ...
    def test_j_possible_reshard(self):
    ...
    ######################### KVS tests #######################################################
    def test_k_delete_key_value_operation(self):
    ...
    def test_l_delete_view_operation(self):

        print("\n###################### Deleting the new node from the view ######################\n")
        # stop and remove node7
        stopAndRemoveInstance("node7")
        time.sleep(50)
