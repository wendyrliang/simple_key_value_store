...
def runReplica(hostPort, ipAddress, subnetName, instanceName):
    command = "docker run -d -p " + hostPort + ":8080 --net=" + subnetName + " --ip=" + ipAddress + " --name=" + instanceName + " -e SOCKET_ADDRESS=" + ipAddress + ":8080" + " -e VIEW=" + view + " -e PYTHONUNBUFFERED=0 assignment3-img"
    os.system(command)
    time.sleep(20)
...
class TestHW3(unittest.TestCase):
    ...
    def test_a_view_operations(self):
        ...
        response = requests.put('http://localhost:8082/key-value-store/key1', json={'value': "value1", "causal_metadata": ""})
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/key2', json={'value': "value2", "causal_metadata": ""})
        self.assertEqual(response.status_code, 201)
    ...
    def test_b_availability(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/key', json={"value": "myvalue", "causal_metadata": ""})
        responseInJson = response.json()

        put_version = responseInJson["version"]
        put_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 201)
    ...
    def test_c_key_value_operations(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/mykey1', json={"value": "myvalue1", "causal_metadata": ""})
        responseInJson = response.json()
        ...
        first_get_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 200)
        ...
        responseInJson = response.json()
        print(responseInJson.get('version'))

        second_get_value = responseInJson['value']
        second_get_version =  responseInJson["version"]
        second_get_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 200)
        ...
        third_get_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 200)
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/mykey1', json={"value": "myvalue2", "causal_metadata": third_get_causal_metadata})
        responseInJson = response.json()
    ...
    def test_f_delete_operations(self):
        ...
        response = requests.put('http://localhost:8082/key-value-store/key1', json={'value': "value1", "causal_metadata": ""})
        responseInJson = response.json()
        replica1_matadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 201)


        print("\n###################### Putting key2/value2 to the store ######################\n")
        # put a new key in the store
        response = requests.put('http://localhost:8083/key-value-store/key2', json={'value': "value2", "causal_metadata": ""})
        replica2_matadata = response.json().get('causal_metadata')
        self.assertEqual(response.status_code, 201)

        print("\n###################### Waiting for 10 seconds ######################\n")
        time.sleep(10)

        print("\n###################### Putting key3/value3 to the store ######################\n")
        # put a new key in the store
        response = requests.put('http://localhost:8084/key-value-store/key3', json={'value': "value3", "causal_metadata": ""})
        replica3_metadata = response.json().get('causal_metadata')
        self.assertEqual(response.status_code, 201)
        ...
        print("\n###################### Getting key1 from the replica2 ######################\n")
        # get the value of the new key from replica1 after putting the new key
        response = requests.get('http://localhost:8083/key-value-store/key1')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 200)
        ...
        print("\n###################### Getting key2 from the replica3 ######################\n")
        # get the value of the new key from replica2 after putting the new key
        response = requests.get('http://localhost:8084/key-value-store/key2')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 200)
        ...
        print("\n###################### Getting key3 from the replica1 ######################\n")
        # get the value of the new key from replica3 after putting the new key
        response = requests.get('http://localhost:8082/key-value-store/key3')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(responseInJson['value'], 'value3')


        print("\n###################### Deleting key1/value1 from the store ######################\n")
        # delete a key from the store
        response = requests.delete('http://localhost:8084/key-value-store/key1', json={"causal_metadata": replica3_metadata})
        self.assertEqual(response.status_code, 200)

        print("\n###################### Deleting key2/value2 from the store ######################\n")
        # delete a key from the store
        response = requests.delete('http://localhost:8082/key-value-store/key2', json={"causal_metadata": replica1_matadata})
        self.assertEqual(response.status_code, 200)

        print("\n###################### Deleting key3/value3 from the store ######################\n")
        # delete a key from the store
        response = requests.delete('http://localhost:8083/key-value-store/key3', json={"causal_metadata": replica2_matadata})
        self.assertEqual(response.status_code, 200)


        print("\n###################### Getting keys from the replicas ######################\n")
        # get the value of the new key from replica1 after putting the new key
        response = requests.get('http://localhost:8082/key-value-store/key1')
        responseInJson = response.json()
        ...
        # get the value of the new key from replica3 after putting the new key
        response = requests.get('http://localhost:8084/key-value-store/key3')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 400)

        
        print("\n###################### Deleting non-existing key from the replicas ######################\n")
        # delete non-existing key
        response = requests.delete('http://localhost:8082/key-value-store/mykey')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 400)
        ######################### Run tests #######################################################
    def test_d_causal_consistency(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/k1', json={"value": "foo", "causal_metadata": ""})
        responseInJson = response.json()

        first_put_version = responseInJson["version"]
        first_put_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 201)
        ...
        response = requests.put('http://localhost:8083/key-value-store/k1', json={"value": "bar", "causal_metadata": first_put_causal_metadata})
        responseInJson = response.json()
        print("responseInJson", responseInJson)

        second_put_version = responseInJson["version"]
        second_put_causal_metadata = responseInJson['causal_metadata']
        self.assertEqual(response.status_code, 200)
    ...
    # this test will check kvs delete
